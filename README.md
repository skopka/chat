...
импорт явно помечен как recovery-key-authenticated, а не повторно проверенная подпись
отправителя. Подключение и ограничения — [резервные копии](docs/backups.md).
Форматы envelope/content/binding не изменены. Для backup требуется явное подключение
новых серверных маршрутов и отдельная PostgreSQL migration; обычный чат продолжает
работать без них. Согласованный набор по-прежнему состоит из 23 пакетов.
...
- [Зашифрованные резервные копии и восстановление истории](docs/backups.md)
