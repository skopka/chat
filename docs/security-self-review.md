...
- The same bounded fuzz harness instruments the versioned client content decoder. Text/reaction content-v1 and attachment content-v2 have pinned bytes; unknown version/kind/flags, truncated identifiers, malformed UTF-8, inconsistent framing, self-reference, empty reaction tokens and oversize input are rejected without reflecting plaintext in exceptions.
...
- `Skopka.Chat.UI.Core` references Client only; `Skopka.Chat.UI.Blazor` references UI.Core and the ASP.NET Core shared framework but not Server or Persistence. Default Razor rendering HTML-encodes message/attachment text, expected send failures retain no remote error text, forwarding requires host target selection, and attachment retrieval is a host callback rather than an automatically embedded URL.
