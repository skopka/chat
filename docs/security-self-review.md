...
- `Skopka.Chat.Server.Kafka` is optional and references Server plus Confluent.Kafka; the Server assembly itself has no Kafka dependency, and the adapter never references Client or PostgreSQL implementation types.
- `Skopka.Chat.Persistence.PostgreSql` models public devices, conversation metadata, ciphertext, tag, signature, delivery state and metadata-only outbox rows; an automated model test rejects plaintext/private-key property names.
...
- Server-event tests pin metadata-only event-v1 JSON, prove the Server assembly has no Kafka dependency, and cover publisher failure/backoff and controlled cancellation. The PostgreSQL gate proves envelope/outbox atomicity and stable-`EventId` redelivery after process loss between broker publish and completion marking.
...
15. **Kafka is an at-least-once metadata notification boundary.** A crash after broker acknowledgement and before outbox completion produces a duplicate `EventId`; producer idempotence does not make consumer effects exactly once. The repository gate does not certify broker ACL/TLS, multi-node ISR/failover, retention, consumer inbox transactions, or sustained load. Severity: medium operational risk and high metadata risk if ACLs are broad. Resolution: durable consumer deduplication before offset commit, least-privilege topics, encrypted transport, bounded retention, backlog/lag alerts, and deployment-specific failure tests.
...
- Kafka ACL/TLS, topic provisioning, producer/consumer retry, durable inbox deduplication, ISR/failover and backlog recovery tests under production-like load when the optional adapter is enabled.
